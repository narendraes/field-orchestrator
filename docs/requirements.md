...
### R18 — Batch before calculation and distinguish latency from capacity

Manifest-admitted source signals must be durable and uniquely identified before consolidation. Process bounded inbox batches, share discovery reads, and avoid a secondary self-contending queue hop for the primary target. Preserve late events, retries, query-lag safety and combined-field validation. Expose discovery and inbox batch measurements separately from target service time. Scale acceptance must include many independent targets and concurrent population, not only many sources feeding one target. Fixed target-owned writer lanes remain planned until all writers and migration are coordinated; see [performance design](performance-design.md).
