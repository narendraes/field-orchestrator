...
## R18 inbox and inline primary processing

New ingress writes `relationship-inbox:v1:<uuid>` with identifiers/timestamp only, then queues `{inboxId}`. No shared read/modify/write occurs at ingress. Worker reads its own key directly, then a beginsWith query capped at ten records. It routes those events with invocation-local read caches and accumulates per-target pending references without scheduling another job for the first target. Extra targets are durably queued. The first consolidated target is computed under the already-held writer slot. Inbox records are deleted only after inline processing and continuation dispatch succeed; replay suppresses equivalent writes. Old event/flush queue bodies remain supported.

Telemetry adds `inboxBatchSize`, `discoveryMs`, `discoveryRequests`; target request counts remain shared per batch. `beforeJobMs` now describes time from first ingress to the inbox worker start for the primary target. Continuations may include earlier processing. End-to-end timing still excludes time before Forge ingress. The global writer lock and population coordination are unchanged. There is no one-second guarantee or independent-target concurrency claim.

Storage failure propagates for retry. Query-lag omissions keep their original queued wake. A failed enqueue leaves an inbox record for later wake-up; with no future wake/retry an operator repair remains necessary. No unattended TTL deletion is introduced. Rollback must drain both inbox and pending-target jobs before deploying handlers that do not understand them. See [capacity and lane design](performance-design.md).
